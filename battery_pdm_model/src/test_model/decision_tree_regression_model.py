...
TRAIN_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "raw_data" / "train" / "Training_Data.csv"
TEST1_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "raw_data" / "test" / "WeldingTest_01_OK.csv"
TEST2_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "raw_data" / "test" / "WeldingTest_02_OK.csv"
TEST3_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "raw_data" / "test" / "WeldingTest_03_NG.csv"
TEST4_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "raw_data" / "test" / "WeldingTest_04_NG.csv"
TEST3_LABEL_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "preprocessed" / "test" / "WeldingTest_03_NG_Label.csv"
TEST4_LABEL_DATA_PATH = ROOT_PATH / "배터리 예지보전 모델" / "data" / "preprocessed" / "test" / "WeldingTest_04_NG_Label.csv"

# decision + 회귀 -> 잔차 -> Z-score -> anomaly_pred 
# 학습용 데이터
train_df = pd.read_csv(TRAIN_DATA_PATH)

# 테스트용 데이터
test_df1 = pd.read_csv(TEST1_DATA_PATH)
test_df2 = pd.read_csv(TEST2_DATA_PATH)
test_df3 = pd.read_csv(TEST3_DATA_PATH)
test_df4 = pd.read_csv(TEST4_DATA_PATH)

# 라벨 데이터
label_df3 = pd.read_csv(TEST3_LABEL_DATA_PATH)
label_df4 = pd.read_csv(TEST4_LABEL_DATA_PATH)

# 컬럼정의
target_column = "RealPower"
feature_columns = ["Speed", "Length", "SetPower", "GateOnTime"]
time_column = "WorkingTime"  # 정렬/그래프 x용
Z_THRESHOLD = 4.0  # 이상 판정 임계값 (가이드북 68p/75p/81p 기준)
...
# NG 파일은 라벨 파일을 '행 순서'로 맞춰 붙임 (PageNo는 39개 단위로 반복되므로 merge 불가)
...
# Decision Tree (회귀 = 이상 범위가 있는지 확인)
...
y_true_label = test_model_df["label"].values  # 실제 양품/불량
...
# 분류 (회귀 → 잔차 → Z-score → anomaly_pred)
# Z-score 기준(mu, sigma)은 테스트셋이 아니라 '학습 데이터(정상)'의 잔차로 고정한다.
# (테스트셋 자체 기준으로 하면 지속형 이상(04번 같은 패턴)을 놓칠 수 있음이 실험으로 확인됨)
# ---------------------------------------------------------------
