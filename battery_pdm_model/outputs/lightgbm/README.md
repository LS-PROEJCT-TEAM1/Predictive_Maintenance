...
| `inspection_reason` (점검 사유) | 출력 0W → 미출력 (레이저 발진·게이트 점검) · 출력이 다른 SetPower 그룹 수준 → 설정-출력 불일치 · 출력 < PageNo 중앙값 → 출력 저하 (광학계·렌즈·레이저 소스) · 출력 > PageNo 중앙값 → 출력 과다 (출력 제어부·설정값) · 학습에 없는 SetPower면 끝에 "+ 출력 설정값(레시피) 확인" 추가 |
